...
int bitset::count() const {
  ...
  for (int i = 0; i < N; i++) {
    ...
    for (int j = 0; temp; temp >>= 1) count += temp & 1;
  }
  return count;
}  //输出一共有多少个为1的位
...
bool bitset::any() const { return count() != 0 ? true : false; }
//是否有是1的位
bool bitset::none() const { return count() == 0 ? true : false; }
//是否没有是1的位
bool bitset::all() const { return count() == 32 * 5 ? true : false; }
